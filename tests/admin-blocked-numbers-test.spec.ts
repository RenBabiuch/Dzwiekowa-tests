...
test('Complete blocking phone numbers works', async({page}) => {
    ...
    await test.step('Go to create first reservation', async() => {
        await page.goto('');
        await pages.reservationPage.reservationForm.fillTheReservationForm('Browar Miesczanski', 'Zespół', reservation.bandName, blockedNumber, reservation.startHour, endHour, reservation.date);
    ...
    await test.step('Go to create reservation with blocked number - the phone number error message should appear', async() => {
        await page.goto('');
        await page.reload();
        await pages.reservationPage.reservationForm.fillTheReservationForm('Browar Miesczanski', 'Zespół', reservation.bandName, blockedNumber, reservationNewStartHour, reservationNewEndHour, reservation.date);
        await pages.reservationPage.submitWithOnlinePayment();
